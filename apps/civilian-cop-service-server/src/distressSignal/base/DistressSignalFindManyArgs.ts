/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DistressSignalWhereInput } from "./DistressSignalWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { DistressSignalOrderByInput } from "./DistressSignalOrderByInput";

@ArgsType()
class DistressSignalFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => DistressSignalWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => DistressSignalWhereInput, { nullable: true })
  @Type(() => DistressSignalWhereInput)
  where?: DistressSignalWhereInput;

  @ApiProperty({
    required: false,
    type: [DistressSignalOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [DistressSignalOrderByInput], { nullable: true })
  @Type(() => DistressSignalOrderByInput)
  orderBy?: Array<DistressSignalOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { DistressSignalFindManyArgs as DistressSignalFindManyArgs };
